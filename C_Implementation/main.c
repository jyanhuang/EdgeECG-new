...
// float* npConv_one_filter1D(float* img, int length, float* weight_one_filter, int kernel_size, int first) {
//     int factor;
//     if(first != -1 ){
//         factor = 256.0;
//     } else{
...
//             int weight_index = m;
//             int input_index = i + m;
//             output_value += weight_one_filter[weight_index] * img[input_index] / factor;
...
// float* npConv1D(float* feature, int length, float* weight, float* bias, int in_channels, int out_channels, int kernel_size, int padding, int group, int first) {
...
//                 float* conv_result = npConv_one_filter1D(newfeature, length, weight_one_filter, kernel_size, first);
...
//             float* conv_result = npConv_one_filter1D(newfeature, length, weight_one_filter, kernel_size, first);
//             for (int j = 0; j < length; j++) {
//                 result[i * length + j] = conv_result[j] + bias[i]/ 256;
//             }
//             free(weight_one_filter);
//             free(newfeature);
//             free(conv_result);
...
//         for (int i = 0; i < inchannel; i++) {
//             for (int j = 0; j < length; j++) {
//                 output[k] += input_data[i * length + j] * weights[k * length * inchannel + i * length + j] / 256.0;
//             }
//         }

//         output[k] += bias[k] / 256.0;
//     }
//     return output;
// }
...
//     readCSV("./conv1_weight.txt", conv1_weight, 15);
//     readCSV("./conv1_bias.txt", conv1_bias, 1);
//     float* conv1 = npConv1D(signal, 300, conv1_weight, conv1_bias, 1, 1, 15, 7, -1, -1);
...
//     readCSV("./conv2_weight.txt", conv2_weight, 17);
//     readCSV("./conv2_bias.txt", conv2_bias, 1);
//     float* conv2 = npConv1D(conv1, 150, conv2_weight, conv2_bias, 1, 1, 17, 8, -1, 0);
...
//     readCSV("./conv3_weight.txt", conv3_weight, 38);
//     readCSV("./conv3_bias.txt", conv3_bias, 2);
//     float* conv3 = npConv1D(conv2, 75, conv3_weight, conv3_bias, 1, 2, 19, 9, -1, 0);
...
//     readCSV("./conv4_weight.txt", conv4_weight, 42);
//     readCSV("./conv4_bias.txt", conv4_bias, 2);
//     float* conv4 = npConv1D(conv3, 38, conv4_weight, conv4_bias, 2, 2, 21, 10, 0, 0);
//     free(conv4_weight);
//     free(conv4_bias);
//     free(conv3);
...
//     readCSV("./conv5_weight.txt", conv5_weight, 8);
//     readCSV("./conv5_bias.txt", conv5_bias, 4);
//     float* conv5 = npConv1D(conv4, 38, conv5_weight, conv5_bias, 2, 4, 1, 0, -1, 0);
//     free(conv5_weight);
//     free(conv5_bias);
//     free(conv4);
...
//     readCSV("./fc1_weight.txt", fc1_weight, 2432);
//     readCSV("./fc1_bias.txt", fc1_bias, 16);
...
//     readCSV("./fc2_weight.txt", fc2_weight, 80);
//     readCSV("./fc2_bias.txt", fc2_bias, 5);
...
//     readCSV("./Y_test_np.txt", labels, totalnum);
//     clock_t start = clock();
...
//     double time_taken = ((double) (end - start)) / CLOCKS_PER_SEC; // 计算运行时间
//     printf("run time: %f s\n", time_taken); // 打印运行时间
...
float* npMaxPool1D(float* img, int length, int kernel_size, int stride, int padding, int inchannel) {
    ...
    for (int i = 0; i < inchannel; i++) {
        for (int j = 0; j < out_length; j++) {
            ...
            float max = img[i * length + ((start < 0) ? 0 : start)];
            for (int k = (start < 0) ? 0 : start; k < ((end > length) ? length : end); k++) {
                float current_val = img[i * length + k];
                if (current_val > max) {
                    max = current_val;
                }
            }
...
float* np_nn(float* signal) {
    float* conv1 = npConv1D(signal, 300, conv1_weight, conv1_bias, 1, 1, 15, 7, -1, -1);
    float* conv1_pool = npMaxPool1D(conv1, 300, 3, 2, 1, 1);
    free(conv1);

    float* conv2 = npConv1D(conv1_pool, 150, conv2_weight, conv2_bias, 1, 1, 17, 8, -1, 0);
    float* conv2_pool = npMaxPool1D(conv2, 150, 3, 2, 1, 1);
    free(conv2);
    free(conv1_pool);

    float* conv3 = npConv1D(conv2_pool, 75, conv3_weight, conv3_bias, 1, 2, 19, 9, -1, 0);
    float* conv3_pool = npAvgPool1D(conv3, 75, 3, 2, 1, 2);
    free(conv3);
    free(conv2_pool);

    float* conv4 = npConv1D(conv3_pool, 38, conv4_weight, conv4_bias, 2, 2, 21, 10, 0, 0);
    free(conv3_pool);
